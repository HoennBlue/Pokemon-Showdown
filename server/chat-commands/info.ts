...
export const commands: ChatCommands = {
	...
	data(target, room, user, connection, cmd) {
		if (!this.broadcasting && this.cmdToken === '!') {
			const message = this.canTalk(this.message);
		...
		for (const [i, newTarget] of newTargets.entries()) {
			...
			if (newTarget.searchType === 'pokemon' && format.onModifySpecies) {
				...
				try {
					...
					pokemon = format.onModifySpecies.call({dex, clampIntRange} as Battle, dex.getSpecies(newTarget.name), {m: {}, set: set} as Pokemon, format, set.item && dex.getItem(set.item) || undefined);
			...
			case 'item':
				...
				desc = showDetails && item.shortDesc && item.desc;
			...
			case 'move':
				...
				desc = showDetails && move.shortDesc && move.desc;
			...
			case 'ability':
				...
				desc = showDetails && ability.shortDesc && ability.desc;
				buffer += `|raw|${Chat.getDataAbilityHTML(ability, !!desc)}\n`;
				if (showDetails) {
					details = {
						"Gen": ability.gen || "CAP",
					};
					if (ability.locked === true) details["&#10003; Not affected by Gastro Acid"] = "";
	...
	weakness(target, room, user) {
		...
		let mod = Dex.mod(toID(targets[targets.length - 1]));
		if (mod) {
			targets.pop();
			target = targets.join();
		} else {
		...
		let species: {exists: true, name: string, types: string[]} | {exists?: false, types: string[]} = mod.getSpecies(target);
