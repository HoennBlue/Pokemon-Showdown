export const Scripts: ModdedBattleScriptsData = {
	...
	init() {
		...
		Object.keys(abilityToItem).forEach(ability => {
			...
			this.itemCache.set(itemid as ID, Object.assign({}, this.getAbility(ability), {
				id: item.id,
				name: item.name,
	...
	getItem(name) {
		...
		const ability = itemToAbility[toID(name)];
		if (ability) return Object.assign({}, this.dex.getAbility(ability), {ability: ability, onTakeItem: false});
	...
	pokemon: {
		hasItem(item) {
			...
			return item.map(toID).includes(this.item);
		},
		hasAbility(ability) {
