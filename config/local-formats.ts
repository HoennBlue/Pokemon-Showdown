...
export const Formats: (InheritableFormatsData | {section: string, column?: number})[] = [
	...
	},
	{
		...
		banlist: [
			'Pawniard', 'Rufflet', 'Arena Trap', 'Shadow Tag', 'Eviolite', 'Light Ball',
		],
		...
		onModifySpecies(template, target, format, effect) {
			...
			if ((Object.values(template.baseStats) as number[]).reduce((x, y) => x + y) > 350) return;
	...
	},
	{
		...
		banlist: ['Dracovish', 'Dragapult', 'Keldeo', 'Shedinja', 'Zeraora', 'Gorilla Tactics'],
		unbanlist: ['Darmanitan'],
		restricted: [
			'Comatose', 'Contrary', 'Fluffy', 'Fur Coat', 'Huge Power', 'Ice Scales', 'Illusion',
			'Imposter', 'Innards Out', 'Intrepid Sword', 'Libero', 'Neutralizing Gas', 'Parental Bond',
			'Protean', 'Pure Power', 'Simple', 'Speed Boost', 'Stakeout', 'Water Bubble', 'Wonder Guard',
		],
		onValidateSet(set, format) {
			if (this.ruleTable.isRestricted('ability:' + toID(set.ability))) {
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] OU'],
		banlist: ['Kartana', 'Pheromosa', 'Acupressure', 'Thousand Arrows', 'Power Construct'],
		...
		checkLearnset(move, template, lsetData, set) {
			if (!move.isNonstandard && !move.isMax && !this.ruleTable.isRestricted('move:' + move.id) && move.id[0] === template.id[0]) return null;
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Dracovish', 'Hydreigon', 'Shedinja'],
		unbanlist: ['Melmetal'],
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] Ubers', 'Dynamax Clause', 'Moody Clause'],
		...
		checkLearnset(move, template, lsetData, set) {
			...
			const problem = this.checkLearnset(move, set.template);
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Huge Power', 'Uber > 1'],
		unbanlist: ['Uber'],
		onModifySpecies(template, target, format, effect) {
			if (!target.side) return;
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const uber = target.side.team.find(set => {
				return this.dex.getSpecies(set.species).tier === 'Uber';
			}) || target.side.team[0];
		...
		validateSet(set, teamHas) {
			...
			if (!this.format.abilityMap) {
				for (const speciesid in this.dex.data.Pokedex) {
					...
					if (pokemon.num < 1 || pokemon.num > 890) continue;
					...
					for (key in pokemon.abilities) {
						const abilityId = toID(pokemon.abilities[key]);
			...
			const isBaseAbility = Object.values(template.abilities).map(toID).includes(abilityId);
			...
			for (const speciesid of pokemonWithAbility) {
				...
				if (donorTemplate.id !== toID(set.species) && this.ruleTable.isRestrictedSpecies(donorTemplate)) {
					continue;
				}
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] Ubers', 'Dynamax Clause'],
		banlist: ['Eternatus', 'Lunala', 'Zacian-Crowned', 'Moody', 'Shadow Tag', 'Electrify'],
		restricted: [
		...
		onValidateSet(set) {
			...
			if (toID(set.ability) === toID(ability)) return;
	...
	},
	{
		...
		banlist: ['Arctovish', 'Arctozolt', 'Dracovish', 'Dracozolt', 'Huge Power', 'Imposter', 'Normalize', 'Shadow Tag', 'Wonder Guard', 'Mimic', 'Sketch', 'Transform'],
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] Ubers', 'Dynamax Clause'],
		banlist: ['Crawdaunt', 'Darmanitan', 'Darumaka', 'Gastly', 'Arena Trap', 'Drizzle', 'Drought', 'Huge Power', 'Moody', 'Shadow Tag', 'Rain Dance', 'Sunny Day', 'Eviolite', 'Light Ball'],
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const newTemplate = this.dex.deepClone(template);
	...
	},
	{
		...
		pokemon: {
			...
			getSharedPower() {
				...
				if (!this.battle.ruleTable.isRestricted('ability:' + toID(this.baseAbility))) this.side.sharedPower.add(this.baseAbility);
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Dracovish', 'Dragapult', 'Silvally', "King's Rock", 'Razor Fang'],
		unbanlist: ['Melmetal'],
		restricted: ['Acupressure', 'Belly Drum', 'Bolt Beak', 'Double Iron Bash', 'Extreme Speed', 'Electrify', 'Fishious Rend', 'Shell Smash', 'Shift Gear', 'Spore'],
		checkLearnset(move, template, lsetData, set) {
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			if (target.set.ability === 'Drought' && boosts[newTemplate.tier] > 20) newTemplate.tier = 'NUBL';
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] OU'],
		restricted: ['Baneful Bunker', 'Block', 'Copycat', 'Destiny Bond', 'Detect', 'Ingrain', 'King\'s Shield', 'Mean Look', 'move:Metronome', 'Nature Power', 'Obstruct', 'Octolock', 'Parting Shot', 'Protect', 'Roar', 'Skill Swap', 'Spiky Shield', 'Sleep Talk', 'Teleport', 'Whirlwind', 'Wish', 'Yawn'],
		onValidateTeam(team, format, teamHas) {
		...
		validateSet(set, teamHas) {
			...
			if (move.category !== 'Status' || move.status === 'slp' || this.ruleTable.isRestricted('move:' + move.id) || set.moves.map(toID).includes(move.id)) return this.validateSet(set, teamHas);
	...
	},
	{
		...
		restricted: ['Cosmoem', 'Cosmog', 'Dialga', 'Giratina', 'Groudon', 'Ho-Oh', 'Kyogre', 'Kyurem', 'Lugia', 'Lunala', 'Mewtwo', 'Necrozma', 'Palkia', 'Rayquaza', 'Reshiram', 'Solgaleo', 'Xerneas', 'Yveltal', 'Zekrom', 'Zygarde'],
		minSourceGen: 7,
		onValidateTeam(team) {
			let n = 0;
			const problems = [];
	...
	},
	{
		...
		validateTeam(team, removeNicknames) {
			if (team.length !== 1 || !['eevee', 'pikachu'].includes(toID(team[0].species))) return ['You may only bring either Pikachu or Eevee.'];
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const accessory = format.accessories![toID(target.set.item!)];
			if (!accessory) return;
			const newTemplate = this.dex.deepClone(template);
			if (!template.types.includes(accessory.type)) newTemplate.types[1] = accessory.type;
			let stat: StatName;
			for (stat in template.baseStats) {
				if (accessory[stat]) newTemplate.baseStats[stat] += accessory[stat];
			}
			return newTemplate;
		},
		onBegin() {
			const accessories = this.format.accessories || {};
			for (const pokemon of this.p1.pokemon.concat(this.p2.pokemon)) {
				if (accessories[toID(pokemon.set.item)]) pokemon.item = 'ultranecroziumz' as ID;
			}
		},
		onSwitchIn(pokemon) {
			const accessories = this.format.accessories || {};
			if (accessories[toID(pokemon.set.item)]) {
				this.add('-start', pokemon, toID(pokemon.set.item), '[silent]');
			}
		},
	},
	{
	...
	},
	{
		...
		unbanlist: ['Greninja-Ash', 'Tapu Lele', 'Arena Trap', 'Psychic Surge'],
		restricted: [],
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			return Object.assign({}, template, {baseStats: {hp: 100, atk: 100, def: 100, spa: 100, spd: 100, spe: 100}});
		},
	},
	{
	...
	},
	{
		...
		onBeforeFaint(pokemon) {
			...
			// XXX Use Validator.checkLearnset
			const learnset = this.dex.getLearnsetData(template.id).learnset || this.dex.getLearnsetData(toID(template.baseSpecies)).learnset || {};
			const prevoset = template.prevo && this.dex.getLearnsetData(toID(template.prevo)).learnset || {};
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
			const validator = new TeamValidator(Dex.getFormat(this.format.id + '@@@' + customRules.join(',')));
			const problems = validator.validateSet(Object.assign({}, set, {ability: ''}), teamHas) || validator.validateSet(Object.assign({}, set, {ability: '', item: set.ability}), teamHas) || [];
			if (toID(set.item) === toID(set.ability)) problems.push((set.name || this.dex.getSpecies(set.species).name) + " is limited to 1 of " + this.dex.getItem(set.item).name);
			else if (toID(set.item).slice(0, 6) === 'choice' && toID(set.ability).slice(0, 6) === 'choice') problems.push((set.name || this.dex.getSpecies(set.species).name) + " has the combination of " + this.dex.getItem(set.item).name + " and " + dual.name + ", which is banned.");
			return problems.length ? problems : null;
		},
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const uber = target.side.team.find(set => {
				const item = this.dex.getItem(set.item);
				return toID(set.ability) === 'powerconstruct' || toID(set.ability) === 'shadowtag' || this.dex.getSpecies(item.megaEvolves === set.species ? item.megaStone : set.species).tier === 'Uber';
			}) || target.side.team[0];
			const stat = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'][target.side.team.indexOf(target.set)] as StatName;
			const newTemplate = this.dex.deepClone(template);
			newTemplate.baseStats[stat] = this.dex.getSpecies(uber.species).baseStats[stat];
	...
	},
	{
		...
		onModifySpecies(template, pokemon, format, effect) {
			...
			return Object.assign({}, template, {addedType: pokemon.hpType});
		},
	},
	{
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			for (const speciesid of pokemonWithAbility) {
				...
				if (donorTemplate.id !== toID(set.species) && this.ruleTable.isRestrictedSpecies(donorTemplate)) {
					continue;
				} else if (donorTemplate.id !== toID(set.species) && this.ruleTable.isRestricted('ability:' + abilityId)) {
					continue;
				}
				set.name = set.species = donorTemplate.name;
				if (!this.validateSet(set)) {
	...
	},
	{
		...
		onBegin() {
			...
			for (const pokemon of allPokemon) {
				...
				pokemon.m.innates = (Object.keys(pokemon.species.abilities) as AbilityIndex[]).filter(key => key !== 'S' && (key !== 'H' || !pokemon.species.unreleasedHidden)).map(key => toID(pokemon.species.abilities[key])).filter(ability => ability !== pokemon.ability && !this.ruleTable.get('-ability:' + ability));
			}
		},
		onSwitchInPriority: 2,
		onSwitchIn(pokemon) {
	...
	},
	{
		...
		checkLearnset(move, template, lsetData, set) {
			if (!set?.sketchMove || move.isZ || this.ruleTable.isRestricted('move:' + move.id)) return this.checkLearnset(move, template, lsetData, set);
			if (move.id === set.sketchMove || !this.checkLearnset(move, template, lsetData, set)) return null;
	...
	},
	{
		...
		ruleset: ['[Gen 7] OU'],
		validateSet(set, teamHas) {
			const effectiveSet = Object.assign({}, set);
			const stats = ['atk', 'attack', 'def', 'defense', 'spa', 'specialattack', 'spd', 'specialdefense', 'spe', 'speed'];
			effectiveSet.moves = set.moves.filter(move => !stats.includes(toID(move)));
		...
		onSwitchIn(pokemon) {
			...
			for (const move of pokemon.set.moves) {
				const index = stats.indexOf(toID(move)) | 1;
				if (index >= 0) {
	...
	},
	{
		...
		ruleset: ['[Gen 7] OU'],
		banlist: ['Slaking', 'Regigigas'],
		restricted: ['Assist', 'Baneful Bunker', 'Block', 'Copycat', 'Destiny Bond', 'Detect', 'Mat Block', 'Mean Look', 'Nature Power', 'Protect', 'Roar', 'Skill Swap', 'Spider Web', 'Spiky Shield', 'Whirlwind', 'Yawn'],
		onValidateTeam(team, format, teamHas) {
			for (const trademark in teamHas.trademarks) {
				if (teamHas.trademarks[trademark] > 1) return ['You are limited to 1 of each Trademark. (You have ' + teamHas.trademarks[trademark] + ' of ' + trademark + ').'];
			}
	...
	},
	{
		...
		checkLearnset(move, template, lsetData, set) {
			...
			if (move.type !== typemons.type) return {type: 'oversketched', maxSketches: typemons.type};
			if (typemons.moves.includes(move.id)) return {type: 'incompatible'};
	...
	},
	{
		...
		onValidateSet(set) {
			...
			const item = this.dex.getItem(set.item);
			if (item.megaEvolves === template.name) template = this.dex.getSpecies(item.megaStone);
			if (toID(set.ability) === 'battlebond' && template.id === 'greninja') template = this.dex.getSpecies('greninjaash');
			...
			if (Object.values(template.baseStats).reduce((x, y) => x + y) > 625) return [`${set.name} (${template.name}) has a BST greater than 625.`];
			if (set.moves.length !== 1 || toID(set.moves[0]) !== 'metronome') return [`${set.name} (${template.name}) has an illegal moveset (only Metronome is allowed).`];
		},
	},
	{
	...
	},
	{
		...
		ruleset: ['[Gen 7] Partners in Crime', '[Gen 7] AAA Doubles'],
		banlist: [],
		unbanlist: [],
		restricted: [],
		onSwitchIn() {},
	...
	},
	{
		...
		checkLearnset(move, template, lsetData, set) {
			...
			if (set?.fuseTemplate && !this.checkLearnset(move, set.fuseTemplate)) return null;
			return this.checkLearnset(move, template, lsetData, set);
		},
		validateSet(set, teamHas) {
		...
		onModifySpecies(template, target, format, effect) {
			...
			if (fuseTemplate.abilities[0] !== ability) mixedTemplate.innate = toID(fuseTemplate.abilities[0]);
			return mixedTemplate;
		},
		onBegin() {
			const allPokemon = this.p1.pokemon.concat(this.p2.pokemon);
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (toID(set.ability) === toID(species.abilities['H'])) return [species.name + "'s ability " + species.abilities['H'] + " is not available in PokeMMO."];
		},
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const baseStats = Object.assign({}, template.baseStats);
			const stats = Object.values(baseStats).slice(+template.battleOnly);
	...
	},
	{
		...
		onTryHit(target, source, move) {
			if (target !== source && !target.ignoringItem()) {
				...
				if (item.onSourceModifyDamage && item.naturalGift && item.naturalGift.type === move.type && target.runEffectiveness(move) > 0 && this.runEvent('TryEatItem', target, source, null, item)) {
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			for (const move of pokemon.set.moves) {
				pokemon.addVolatile('ability:' + toID(move));
			}
		},
	},
	{
		...
		checkLearnset(move, template, lsetData, set) {
			...
			if (!this.ruleTable.isRestricted('ability:' + ability.id)) {
				for (const speciesid in this.dex.data.Learnsets) {
					...
					if (this.checkLearnset(move, this.dex.getSpecies(speciesid))) continue;
					return null;
				}
			}
			return this.checkLearnset(move, template, lsetData, set);
	...
	},
	{
		...
		onValidateSet(set) {
			const template = this.dex.getSpecies(set.species || set.name);
			if (toID(set.ability)[0] !== template.id[0]) {
				let legalAbility = false;
				let i: AbilityIndex;
				for (i in template.abilities) {
					if (set.ability === template.abilities[i]) legalAbility = true;
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			for (const statName in newTemplate.baseStats) {
				if (newTemplate.baseStats[statName] <= 70) newTemplate.baseStats[statName] *= 2;
			}
			return newTemplate;
		},
	},
	...
	},
	{
		...
		onAfterMega(pokemon) {
			if (pokemon.m.innate === toID(pokemon.ability)) {
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const baseStats: SparseStatsTable = {};
			let stat: StatName;
			for (stat in template.baseStats) {
	...
	},
	{
		...
		validateSet(set, teamHas) {
			if (toID(set.ability) !== 'galewings') return this.validateSet(set, teamHas);
			const customRules = this.format.customRules || [];
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			for (const speciesid of pokemonWithAbility) {
				...
				if (donorTemplate.id !== toID(set.species) && this.format.customBans!.donor.includes(donorTemplate.id)) {
					continue;
				} else if (donorTemplate.id !== toID(set.species) && this.format.customBans!.inheritedAbilities.includes(abilityId)) {
					continue;
				}
				set.name = set.species = donorTemplate.name;
				if (!this.validateSet(set)) {
	...
	},
	{
		...
		onValidateSet(set) {
			const template = this.dex.getSpecies(set.species || set.name);
			set.level = Math.round(Object.values(template.baseStats).reduce((x, y) => x - y, 1000) / 100);
		},
	},
	{
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			const problems = this.validateSet(Object.assign({}, set, {item: ''}), teamHas);
			if (problems) return problems;
			const customRules = this.format.customRules || [];
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
			return new TeamValidator(Dex.getFormat(this.format.id + '@@@' + customRules.join(','))).validateSet(Object.assign({}, set, {ability: set.item, item: ''}, teamHas));
		},
		onValidateSet(set) {
			const ability = this.dex.getAbility(set.ability);
			const item = this.dex.getItem(set.item);
	...
	},
	{
		...
		onValidateSet(set, format, setHas, teamHas) {
			...
			if (teamHas['move:' + toID(set.moves[0])] > 2) return ['Your team has the combination of ' + this.dex.getMove(set.moves[0]).name + ' > 2, which is banned.'];
		},
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const item = this.dex.getItem(target.set.item);
			if (item.onPlate && !template.types.includes(item.onPlate)) return Object.assign({}, template, {addedType: item.onPlate});
		},
		onSwitchInPriority: 2,
		onSwitchIn(pokemon) {
	...
	},
	{
		...
		checkLearnset(move) {
			return move.id === 'metronome' ? null : {type: 'invalid'};
		},
		validateSet(set, teamHas) {
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			return Object.assign({}, template, {types: [template.types[0]]});
		},
	},
	{
		name: "[Gen 7] SlowMons",
	...
	},
	{
		...
		onModifySpecies(template, target, effect) {
			const item = this.dex.getItem(target.set.item);
			if (effect && item.type && !template.types.includes(item.type)) return Object.assign({}, template, {types: [template.types[0], item.type]});
		},
		onModifyMovePriority: -2,
		onModifyMove(move, pokemon, target) {
	...
	// Ability Shift
	{
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				...
				if (this.ruleTable.isRestricted('ability:' + toID(ability))) {
					let legalAbility = false;
					let i: AbilityIndex;
					for (i in template.abilities) {
	...
	},
	{
		...
		ruleset: ['[Gen 5] Ubers', 'Evasion Moves Clause'],
		unbanlist: [],
	},
	{
		name: "[Gen 4] 1v1",
	...
	},
	{
		...
		onBegin() {
			...
			for (const pokemon of allPokemon) {
				...
				for (let j = 0; j < pokemon.moveSlots.length; j++) {
					...
					const customRenamedSet = customRenamedMoves[toID(pokemon.name)];
		...
		onSwitchInPriority: 1,
		onSwitchIn(pokemon) {
			const name = toID(pokemon.illusion ? pokemon.illusion.name : pokemon.name);
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			if (this.format.accessories![toID(pokemon.set.item)]) {
				this.add('-start', pokemon, toID(pokemon.set.item), '[silent]');
				this.add('-start', pokemon, 'typechange', pokemon.types.join('/'));
			}
		},
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			case 'pawn':
				set.name = 'Pawn';
				format = '[Gen 6] LC';
				format = Object.assign({}, Dex.getFormat(format), {maxLevel: 100});
				break;
			case 'knight':
				set.name = 'Knight';
				break;
		...
		onValidateTeam(team) {
			...
			if (pieces.has('Knight')) {
				...
				else if (knight.prevo !== toID(pieces.get('Pawn').species) && this.dex.getSpecies(knight.prevo).prevo !== toID(pieces.get('Pawn').species)) problem = '' + knight.name + ' does not evolve from ' + pieces.get('Pawn').species + '.';
	...
	},
	{
		...
		checkLearnset(move, template, lsetData, set) {
			return move.gen !== template.gen && (!template.prevo || this.format.checkLearnset!.call(this, move, this.dex.getSpecies(template.prevo), lsetData, set)) && this.checkLearnset(move, template, lsetData, set);
		},
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const uber = target.side.team.find(set => {
				const item = this.dex.getItem(set.item);
				return toID(set.ability) === 'shadowtag' || this.dex.getSpecies(item.megaEvolves === set.species ? item.megaStone : set.species).tier === 'Uber';
			}) || target.side.team[0];
			const stat = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'][target.side.team.indexOf(target.set)] as StatName;
			const newTemplate = this.dex.deepClone(template);
			newTemplate.baseStats[stat] = this.dex.getSpecies(uber.species).baseStats[stat];
	...
	},
	{
		...
		onBegin() {
			const allPokemon = this.p1.pokemon.concat(this.p2.pokemon);
			for (const pokemon of allPokemon) {
				pokemon.m.mirrorMoveSlots = pokemon.baseMoveSlots.map(move => Object.assign({}, move, {disabled: false, disabledSource: ''}));
			}
		},
	...
	},
	{
		...
		banlist: ['Fletchling'],
		checkLearnset(move, template, lsetData, set) {
			return template.evos.every(evo => this.format.checkLearnset!.call(this, move, this.dex.getSpecies(evo), lsetData, set)) ? this.checkLearnset(move, template, lsetData, set) : null;
		},
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			return Object.assign({}, template, {baseStats: this.dex.getSpecies(pal.species).baseStats});
		},
	},
	{
	...
	},
	{
		...
		ruleset: ['[Gen 6] OU'],
		banlist: ['Slaking', 'Regigigas'],
		restricted: ['Assist', 'Block', 'Copycat', 'Detect', 'King\'s Shield', 'Mat Block', 'Mean Look', 'Protect', 'Roar', 'Skill Swap', 'Spiky Shield', 'Whirlwind'],
		onValidateTeam(team) {
			let count = 0;
			for (const set of team) {
				const trademark = toID(set.ability);
		...
		validateSet(set, teamHas) {
			const move = this.dex.getMove(set.ability);
			if (move.category !== 'Status' || this.ruleTable.isRestricted('move:' + move.id) || set.moves.map(toID).includes(move.id) || this.checkLearnset(move, this.dex.getSpecies(set.species), undefined, set)) return this.validateSet(set, teamHas);
			set.ability = '';
			const customRules = this.format.customRules || [];
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (/BL$/.test(tier)) tier = ({UUBL: 'OU', RUBL: 'UU', NUBL: 'RU', PUBL: 'NU'} as {[k: string]: string})[tier];
